...
<tr><td align="center"><table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#fff;border-radius:10px;overflow:hidden">
  ...
  if (req.method === 'POST' && url === '/api/ac/ci-generate-r2') {
    ...
    try {
      // Busca resultado + dados do cliente
      const r = await pool.query(`
        SELECT res.*, c.name as client_name, c.email as client_email,
               ct.test_id, ct.id as client_test_id_val,
               COALESCE(m.slug, ct.test_id) as module_slug
      ...
      // Se já existe e não foi pedida regeneração, retorna o salvo
      if (row.ai_analysis_r2 && !forceRegen) {
        return json(200, { ok:true, text: row.ai_analysis_r2, cached: true });
      }

      const scores = JSON.parse(row.scores_json || '{}');
      const seg = scores.seguranca     || 0;
      const val = scores.validacao     || 0;
      const per = scores.pertencimento || 0;
      const ide = scores.identidade    || 0;
      const cia = scores.crianca_atual || 0;
      ...
      // Busca respostas individuais com texto das perguntas
      ...
      `, [row.client_test_id_val || row.client_test_id]);
      ...
      if (answersR.rows.length > 0) {
        ...
        for (const [cat, items] of Object.entries(bycat)) {
          ...
          for (const a of items) {
            const scoreLabel = a.score>=4?'Sempre/Concordo totalmente':a.score>=3?'Frequentemente/Concordo':a.score>=2?'Às vezes/Neutro':a.score>=1?'Raramente/Discordo':'Nunca/Discordo totalmente';
            respostasDetalhadas += `  • ${a.question||`Pergunta ${a.position}`}: ${a.score}/5 (${scoreLabel})\n`;
          }
        }
      } else {
...
Sua função é gerar um relatório psicanalítico de autoconhecimento PROFUNDO, PERSONALIZADO e com BASE CIENTÍFICA SÓLIDA. O relatório deve ser escrito em português formal, com linguagem terapêutica acolhedora e precisão clínica.
...
RESPOSTAS DETALHADAS DO CLIENTE:
${respostasDetalhadas}

BASES TEÓRICAS OBRIGATÓRIAS (aplique estas proporções na análise):
- Winnicott — 40%: Verdadeiro Self, Falso Self, holding, mirroring, ambiente facilitador, falhas de cuidado, reparentalização.
- Freud — 30%: Inconsciente, mecanismos de defesa, repressão, compulsão à repetição, formação do ego, trauma precoce.
- Jung — 20%: Arquétipo da Criança Interior, sombra, complexos emocionais, individuação, Self como totalidade.
- Bowlby — 10%: Teoria do apego, base segura, apego ansioso/evitativo/desorganizado, medo do abandono, modelos internos de trabalho.

REGRA FUNDAMENTAL: Cada seção do relatório DEVE referenciar explicitamente as pontuações das dimensões e o padrão de respostas do cliente para construir a análise. JAMAIS escreva texto genérico que poderia servir para qualquer pessoa. Toda afirmação deve ser derivada dos dados reais do cliente.

---
GERE O RELATÓRIO COM EXATAMENTE ESTAS SEÇÕES, NESTA ORDEM:

SEÇÃO 1 — RESULTADO GERAL
Escreva 3 a 4 parágrafos contextualizando o resultado de ${row.client_name}. Explique o que a pontuação ${total}/150 significa dentro do espectro do protocolo. Explique a classificação "${classif}" com profundidade científica — o que essa classificação representa segundo Winnicott (conceito de ambiente facilitador e suas falhas), Freud (formação do psiquismo) e Jung (estado de integração ou fragmentação da Criança Interior arquetípica). Mencione especificamente quais dimensões estão mais comprometidas e quais apresentam recursos preservados, baseando-se nas pontuações reais.

SEÇÃO 2 — CLASSIFICAÇÃO DA CRIANÇA INTERIOR
Escreva 4 a 5 parágrafos. Descreva com riqueza clínica o que significa para ${row.client_name} estar nesta classificação. Use linguagem que permita a identificação: descreva sensações, padrões cotidianos, pensamentos internos típicos e a experiência subjetiva de quem vive com esta ferida. Fundamente em Winnicott e Jung. Conclua com uma mensagem de acolhimento que normalize a ferida sem minimizá-la.

SEÇÃO 3 — PONTUAÇÃO POR DIMENSÃO
Para CADA uma das 5 dimensões, escreva um parágrafo analítico completo (mínimo 5 linhas), relacionando a pontuação específica com o referencial teórico correspondente e com o padrão geral de ${row.client_name}.
Segurança Emocional (${seg}/30 — ${dimCls(seg)}): análise via Bowlby e Winnicott.
Validação Emocional (${val}/30 — ${dimCls(val)}): análise via Winnicott e Freud.
Pertencimento (${per}/30 — ${dimCls(per)}): análise via Freud e Bowlby.
Identidade Autêntica (${ide}/30 — ${dimCls(ide)}): análise via Winnicott.
Criança Interior Atual (${cia}/30 — ${dimCls(cia)}): análise via Jung e Winnicott.

SEÇÃO 4 — FERIDA EMOCIONAL CENTRAL
Escreva 5 a 6 parágrafos. Identifique e nomeie a ferida central (ex: Ferida de Abandono, Ferida de Rejeição, Ferida de Humilhação, Ferida de Injustiça, Ferida de Traição) com base na dimensão de menor pontuação. Descreva como ela se manifesta no cotidiano de ${row.client_name}. Se houver ferida secundária (segunda menor pontuação), descreva-a e como as duas interagem. Fundamente em Winnicott, Freud e Jung.

SEÇÃO 5 — ORIGEM EMOCIONAL PROVÁVEL
Escreva 4 a 5 parágrafos descrevendo o ambiente emocional que provavelmente originou esta ferida. Que mensagens verbais e não-verbais ${row.client_name} provavelmente recebeu? Que comportamentos dos cuidadores? Use Winnicott, Freud e Bowlby. Encerre afirmando que esta é uma hipótese interpretativa baseada nos dados do protocolo.

SEÇÃO 6 — NECESSIDADES EMOCIONAIS NÃO ATENDIDAS
Introdução de 2 parágrafos. Em seguida, liste de 6 a 8 necessidades específicas não atendidas para ${row.client_name}, cada uma com parágrafo explicando: (1) qual é a necessidade, (2) como a ausência se manifesta hoje na vida adulta, (3) o fundamento teórico. Conclua com mensagem de esperança terapêutica.

SEÇÃO 7 — PADRÕES DE PROTEÇÃO DESENVOLVIDOS
3 parágrafos de contextualização teórica. Em seguida, 5 a 7 padrões específicos de proteção com: nome claro, como funciona como proteção, como se manifesta no comportamento adulto de ${row.client_name}, e custo emocional de mantê-lo ativo.

SEÇÃO 8 — MECANISMOS DE DEFESA PROVÁVEIS
2 parágrafos introdutórios sobre mecanismos de defesa segundo Freud. Em seguida, 4 a 6 mecanismos com: nome em negrito, definição clínica acessível, como se manifesta especificamente em ${row.client_name}, e exemplos concretos do cotidiano.

SEÇÃO 9 — IMPACTOS NA VIDA ADULTA
4 a 5 parágrafos em texto corrido sobre os impactos nos diferentes domínios da vida adulta de ${row.client_name}: profissional, decisões, corpo, criatividade, prazer, direção de vida. Use Freud (compulsão à repetição) e Winnicott (capacidade de brincar). Inclua parágrafo sobre como esses impactos podem ser confundidos com "personalidade".

SEÇÃO 10 — IMPACTOS NOS RELACIONAMENTOS
4 a 5 parágrafos sobre como a ferida de ${row.client_name} se manifesta nas relações afetivas, familiares e profissionais. Use Bowlby (padrão de apego e como se repete nos relacionamentos adultos), Freud (transferência) e Winnicott (capacidade de intimidade). Descreva o paradoxo central: como ${row.client_name} busca nos relacionamentos aquilo que mais teme.

SEÇÃO 11 — IMPACTOS NA AUTOESTIMA
4 a 5 parágrafos sobre a relação de ${row.client_name} consigo mesmo(a): voz interna crítica, autossabotagem, dificuldade de reconhecer conquistas. Use Freud (superego punitivo), Winnicott (validação e senso de valor) e Jung (sombra). Termine com o que é possível reconstruir.

SEÇÃO 12 — SÍMBOLO DA CRIANÇA INTERIOR
3 a 4 parágrafos poéticos e simbólicos. Baseado no padrão completo de ${row.client_name}, escolha e nomeie um símbolo específico para a Criança Interior. Explique com profundidade. Use Jung (arquétipo da Criança). Termine com uma mensagem direta à Criança Interior de ${row.client_name}, em segunda pessoa, com acolhimento.

SEÇÃO 13 — NÚCLEO PSICANALÍTICO DA FERIDA
5 a 6 parágrafos com profundidade analítica. Articule como os quatro referenciais teóricos se entrelaçam: Winnicott (Verdadeiro Self vs. Falso Self neste caso), Freud (dinâmica inconsciente, repressão, superego), Jung (complexo emocional, sombra, individuação interrompida), Bowlby (modelo interno de trabalho formado). Mostre como se reforçam mutuamente.

SEÇÃO 14 — DIRECIONAMENTO TERAPÊUTICO PRINCIPAL
4 a 5 parágrafos com direcionamento específico para ${row.client_name}. Qual é a prioridade terapêutica? Que tipo de relação terapêutica é necessária? Que abordagens têm maior probabilidade de eficácia? O que ${row.client_name} precisará estar disposto(a) a fazer?

SEÇÃO 15 — RECOMENDAÇÕES
8 a 10 recomendações terapêuticas específicas para ${row.client_name}, cada uma como parágrafo com: (1) nome em negrito, (2) o que é, (3) por que é relevante para este perfil específico, (4) orientação prática inicial de como começar.

SEÇÃO 16 — PLANO DE DESENVOLVIMENTO EMOCIONAL
5 fases, cada uma com: nome, duração estimada, objetivo central, foco terapêutico específico para ${row.client_name}, principais práticas (3 a 5), marcadores de progresso observáveis, e fundamento teórico principal.
Fase 1: Segurança e Reconexão
Fase 2: Escuta e Validação Interna
Fase 3: Reparentalização
Fase 4: Ressignificação e Integração
Fase 5: Autenticidade e Nova Narrativa

SEÇÃO 17 — SÍNTESE FINAL HUMANIZADA
5 a 7 parágrafos em segunda pessoa (falando diretamente com ${row.client_name}), com linguagem íntima e acolhedora. Resuma emocionalmente a jornada revelada pelos dados. Valide a dor sem romantizá-la. Nomeie o que foi perdido na infância. Afirme o que ainda é possível recuperar. Dirija-se à Criança Interior com gentileza. Convoque ${row.client_name} para o processo de cura. Termine com uma frase final poderosa e personalizada.

---
OBSERVAÇÃO ÉTICA (incluir ao final):
⚠️ OBSERVAÇÃO ÉTICA: Este relatório é uma leitura terapêutica de autoconhecimento baseada no Protocolo AXIS IA — Criança Interior, desenvolvido por Clau Diniz. Não constitui diagnóstico psicológico, psiquiátrico ou avaliação clínica. Os conteúdos apresentados são hipóteses interpretativas baseadas nas respostas ao protocolo e devem ser explorados no contexto de um processo terapêutico conduzido por profissional habilitado. Toda análise respeita os princípios éticos do Conselho Federal de Psicologia (CFP).

---
INSTRUÇÕES DE FORMATAÇÃO:
- Use o nome ${row.client_name.split(' ')[0]} de forma natural ao longo do texto — não em todo parágrafo, mas o suficiente para que o relatório seja claramente pessoal.
- Use **negrito** para conceitos teóricos importantes.
- Escreva em português formal, culto e acolhedor.
- O relatório completo deve ter entre 4.000 e 6.000 palavras.
- NÃO use linguagem genérica. Escreva sobre ${row.client_name.split(' ')[0]} especificamente.
- Quando usar linguagem hipotética, use "provavelmente", "os dados sugerem", "o perfil indica".`;
